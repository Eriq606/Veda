...
public class DAO {
    ...
    public <T>T[] select(Connection connex, Class<T> c, String addOn) throws Exception{
        ...
        PreparedStatement statemnt=connect.prepareStatement(QueryUtils.getSelectQuery(c, addOn));
        HashMap<Field, String> columns=QueryUtils.getColumnsWithField(c);
        try{
            LinkedList liste=new LinkedList();
            ResultSet result=statemnt.executeQuery();
    ...
    public <T>T[] select(Connection connex, Class<T> c, T where, int limit, int offset) throws Exception{
        boolean opened=false;
        Connection connect=connex;
        if(connect==null){
            connect=DAOConnexion.getConnexion(driver, server, host, port, database, user, pwd, useSSL, allowKeyRetrieval);
        ...
        PreparedStatement statemnt=connect.prepareStatement(QueryUtils.getSelectQuery(c, where, limit, offset));
    ...
    public <T>T[] select(Connection connex, Class<T> c, T where, String[] order) throws Exception{
        boolean opened=false;
        Connection connect=connex;
        if(connect==null){
            connect=DAOConnexion.getConnexion(driver, server, host, port, database, user, pwd, useSSL, allowKeyRetrieval);
        ...
        PreparedStatement statemnt=connect.prepareStatement(QueryUtils.getSelectQuery(c, where, order));
        Field[] fields=QueryUtils.getNotNullColumns(where);
        statemnt=QueryUtils.mapStatement(statemnt, fields, where);
        HashMap<Field, String> columns=QueryUtils.getColumnsWithField(c);
        try{
